-- Migration: 20251212_tweak_fuzzy.sql
-- Purpose: Tighten fuzzy matching threshold and improve grouping logic

-- Update customer matching with higher threshold (0.7)
CREATE OR REPLACE FUNCTION public.get_customer_match_suggestions(p_threshold float DEFAULT 0.7, p_limit int DEFAULT 20)
RETURNS TABLE (
    master_id text,
    master_name text,
    suggestions jsonb
...
BEGIN
    ...
    pairs AS (
        ...
        FROM unmatched u1
        JOIN unmatched u2 ON u1.customer_id < u2.customer_id
...
-- Update product matching with higher threshold (0.7)
CREATE OR REPLACE FUNCTION public.get_product_match_suggestions(p_threshold float DEFAULT 0.7, p_limit int DEFAULT 20)
RETURNS TABLE (
    master_id bigint,
    master_name text,
    suggestions jsonb
